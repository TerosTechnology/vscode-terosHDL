...
if installation_path != "":
    ...
    if check_dir == True:
        ...
        print(
            "---> Installation folder path: {} doesn't exists. It will search in the system path!!".format(
                installation_path
            )
        )
...
only_synt = False
try:
    if edam["tool_options"]["vivado"]["pnr"] == "none":
        only_synt = True
except:
    pass

backend = edalize.get_edatool(simulator)(edam=edam, work_root=work_root)
...
'
            ...
            p.wait()
    if simulator == "vivado" and only_synt:
        p = subprocess.Popen(["make", "synth"], cwd=work_root)
        p.wait()
    else:
        backend.build()
        backend.run()
