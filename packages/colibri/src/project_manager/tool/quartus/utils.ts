...
export function getQuartusPath(config: e_config): string {
  ...
  if (quartusInstallationPath !== "") {
    if (file_utils.check_if_file(quartusInstallationPath)) {
      quartusInstallationPath = file_utils.get_directory(
        quartusInstallationPath
      );
    }
  ...
  if (QUARTUS_ROOTDIR !== undefined && QUARTUS_ROOTDIR !== "") {
    const quartusRootDir = path_lib.resolve(
      path_lib.join(QUARTUS_ROOTDIR, getBinFolder())
    );
  ...
  if (QSYS_ROOTDIR !== undefined && QSYS_ROOTDIR !== "") {
    const quartusRootDir = path_lib.resolve(
      path_lib.join(QSYS_ROOTDIR, "..", "..", "quartus", getBinFolder())
    );
    return normalizePath(quartusRootDir);
  }
...
export function getQsysPath(config: e_config): string {
  const qsysPath = path_lib.resolve(
    path_lib.join(getQuartusPath(config), "..", "..", "qsys", "bin")
  );
  return qsysPath;
}
 ...
 **/
async function executeQuartusTcl(
  is_sta: boolean,
  config: e_config,
  tcl_file: string,
  args: string,
  ...
  const cmd_result = await new process_teros.Process(undefined).exec_wait(
    cmd,
    options
  );
...
export async function getProjectInfo(
  ...
  const cmd_result = await executeQuartusTcl(
    false,
    config,
    tcl_file,
    args,
 ...
 * @returns Quartus project info.
 **/
export async function getFamilyAndParts(
  config: e_config,
  emitterProject: ProjectEmitter
): Promise<t_board_list[]> {
 ...
 **/
export async function executeCmdListQuartusProject(
  config: e_config,
  projectPath: string,
  cmdList: string[],
  emitterProject: ProjectEmitter
  ...
  const cmdResult = await executeQuartusTcl(
    false,
    config,
    tclFile,
    args,
...
export async function addFilesToProject(
  ...
  for (const file of fileList) {
    let cmd = `set_global_assignment -name ${
      LANGUAGE_MAP[file.file_type]
    } ${escapeBackslashes(file.name)}`;
  ...
  const result = await executeCmdListQuartusProject(
    config,
    projectPath,
    cmd_list,
    emitterProject
 ...
 * @param fileList List of files to add.
 **/
export async function removeFilesFromProject(
  config: e_config,
  projectPath: string,
  fileList: t_file[],
  emitterProject: ProjectEmitter
  ...
  const cmd_list: string[] = [];
  for (const file of fileList) {
    let cmd = `set_global_assignment -remove -name ${
      LANGUAGE_MAP[file.file_type]
    } ${escapeBackslashes(file.name)}`;
    if (file.logical_name !== "") {
      cmd += ` -library ${file.logical_name}`;
    ...
    const fileRelative = escapeBackslashes(
      file_utils.get_relative_path(file.name, projectPath)
    );
    cmd = `set_global_assignment -remove -name ${
      LANGUAGE_MAP[file.file_type]
    } ${fileRelative}`;
    if (file.logical_name !== "") {
      cmd += ` -library ${file.logical_name}`;
 ...
 **/
export async function setTopLevelPath(
  config: e_config,
  projectPath: string,
  topLevelPath: string,
  emitterProject: ProjectEmitter
 ...
 * @param emitterProject Project emitter.
 **/
export async function setTopLevelTestbench(
  config: e_config,
  projectPath: string,
  topLevelTestbenchPath: string,
  emitterProject: ProjectEmitter
): Promise<void> {
  ...
  const cmd = [
    ...
    `set_global_assignment -name EDA_TEST_BENCH_FILE ${escapeBackslashes(
      topLevelTestbenchPath
 ...
 * @param emitterProject Project emitter.
 **/
export async function setTestbenchFileList(
  config: e_config,
  projectPath: string,
  testbenchFileList: t_file[],
  emitterProject: ProjectEmitter
): Promise<void> {
  const cmd = ["remove_all_global_assignments -name EDA_TEST_BENCH_FILE"];
  ...
  for (const file of testbenchFileList) {
    ...
    cmd.push(
      `set_global_assignment -name EDA_TEST_BENCH_FILE ${escapeBackslashes(
        file.name
      )} -section_id testbenchSet ${libCMD}`
    );
  }
 ...
 */
export async function createHTMLReportFromRDB(
  config: e_config,
  rdbPath: string
): Promise<string> {
 ...
 */
export async function createRPTReportFromRDB(
  config: e_config,
  rdbPath: string
): Promise<string> {
...
export async function setConfigToProject(
  config: e_config,
  projectPath: string,
  emitterProject: ProjectEmitter
): Promise<void> {
  ...
  // Optimization mode
  const optimizationMode = config.tools.quartus.optimization_mode
    .toLocaleUpperCase()
    .replace(/_/g, " ");
  cmdList.push(
    `set_global_assignment -name OPTIMIZATION_MODE ${optimizationMode}`
  );
  ...
  // Optimization effort
  await executeCmdListQuartusProject(
    config,
    projectPath,
    cmdList,
    emitterProject
...
export async function openRTLAnalyzer(
  config: e_config,
  projectPath: string,
  emitterProject: ProjectEmitter,
  rtlType: e_rtlType
  ...
  // eslint-disable-next-line max-len
  const cmd = `${quartus_bin} --keep_open -t "${escapeBackslashes(
    tcl_file
  )}" "${projectPath}" "${rtlType}"`;

  const options = { cwd: path_lib.dirname(projectPath) };
  const result = await new process_teros.Process(undefined).exec_wait(
    cmd,
    options
  );
