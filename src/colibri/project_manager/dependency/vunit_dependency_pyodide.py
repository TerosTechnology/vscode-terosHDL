...
for i in range(0, len(nodes)):
    c_node = str(complete_nodes[i])
    cc_node = str(nodes[i])

    for k in range(0, len(project_sources)):
        if c_node == project_sources[k]["name_map"]:
            c_node = project_sources[k]["name"]

        if cc_node == project_sources[k]["name_map"][1:]:
            cc_node = ntpath.basename(project_sources[k]["name"])

    diagram += '    "' + \
        c_node + '" [label="' + cc_node + '"]\n'
...
for i in range(0, len(dependencies)):
    for j in range(0, len(dependencies[i])):
        if (str(complete_nodes[i]) != str(dependencies[i][j])):
            ...
            c_node = str(complete_nodes[i])
            c_dep = str(dependencies[i][j])

            for k in range(0, len(project_sources)):
                if c_node == project_sources[k]["name_map"]:
                    c_node = project_sources[k]["name"]

                if c_dep == project_sources[k]["name_map"]:
                    c_dep = project_sources[k]["name"]
