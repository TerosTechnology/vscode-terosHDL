 ...
 */
function getProjectByName(name: string | undefined, multiProject: Multi_project_manager) {
    try {
        if (name === undefined) {
...
export class Config_manager {
    ...
    private currentConfig: e_config;
    ...
    private getWebviewContent(webview: vscode.Webview): string {
        ...
        const css_codicons = webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'resources',
            'icon', 'codicons', 'codicon.css'));
    ...
    private async loadConfigFromFile(): Promise<void> {
        vscode.window.showOpenDialog({ title: "Load TerosHDL Settings", canSelectMany: false }).then((value) => {
            ...
            const file_content = read_file_sync(path_norm);
    ...
    private emitSaveSettings() {
        if (this.currentConfigIsGlobal) {
            this.emitterProject.emitEvent("", e_event.SAVE_SETTINGS);
        }
    }
}
