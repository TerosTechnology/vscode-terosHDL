...
import { hdl_hover, result_t } from 'colibri/utils/numbers';
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        let hover_numbers_vhdl = vscode.languages.registerHoverProvider(utils.VHDL_SELECTOR, {
            provideHover(document, position, token) {
                return hover(document, position, LANGUAGE.VHDL);
            }
        });
        // Verilog/SV
        let hover_numbers_verilog = vscode.languages.registerHoverProvider(utils.VERILOG_SELECTOR, {
            provideHover(document, position, token) {
                return hover(document, position, LANGUAGE.VERILOG);
            }
        });
...
function hover(document, position, lang: LANGUAGE) {
    ...
    if (wordRange !== undefined) {
        ...
        result = hdl_hover(leadingText, lang);
