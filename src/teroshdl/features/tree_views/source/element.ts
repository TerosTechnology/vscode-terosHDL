...
export class Source_tree_element extends vscode.TreeItem {
    ...
    constructor(element_type: SOURCE_TREE_ELEMENT, name: string, is_manual: boolean,
        ...
        sourceType: e_source_type, children?: any[]) {
        ...
        if (sourceType === e_source_type.SIMULATION) {
            ...
            logical_list.forEach(logical_inst => {
                ...
                if (logical_inst.name !== "") {
                    source_view.push(new Source_tree_element(SOURCE_TREE_ELEMENT.LIBRARY, logical_inst.name, 
                        false, false, logical_inst.name, e_source_type.NONE,
                        children_list));
                }
                else {
...
export class SourcetDecorator implements vscode.FileDecorationProvider {
    ...
    provideFileDecoration(uri: vscode.Uri): vscode.ProviderResult<vscode.FileDecoration> {
        if (uri.path === "/" + e_source_type.SIMULATION) {
            return {
                badge: "T",
                tooltip: "Testbench file",
            };
        }
        else if (uri.path === "/" + e_source_type.SYNTHESIS) {
            return {
                tooltip: "Synthesis file",
            };
        }
        else if (uri.path === "/" + e_source_type.NONE) {
            return {
                tooltip: "Synthesis file",
            };
        }
